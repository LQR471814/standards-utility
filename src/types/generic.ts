...
export class Wrap {
  static Box(o: Box) {
    return {
      ...
      midpoint: function () {
        return [this.x1 + this.width() / 2, this.y1 + this.height() / 2];
      },
    };
  }
  ...
  static TaskState(o: TaskState) {
    return {
      ...
      increment: function (state: boolean): boolean {
        if (this.hands.oneofKind === "handsSeparate") {
          if (
            this.hands.handsSeparate.left.length <
            this.hands.handsSeparate.number
          ) {
            this.hands.handsSeparate.left = [
              ...this.hands.handsSeparate.left,
              state,
        ...
        if (
          ...
          this.hands.handsTogether.completed.length <
            this.hands.handsTogether.number
        ) {
          this.hands.handsTogether.completed = [
            ...this.hands.handsTogether.completed,
            state,

...
export function propertyOnSize(
  node: HTMLElement,
  options: PropertyOnSizeOptions
) {
  const observer = new ResizeObserver(() => {
    for (const property in options) {
      const value =
        options[property]?.axis === "x" ? node.clientWidth : node.clientHeight;

      const threshold =
        typeof options[property]?.threshold === "number"
          ? (options[property]?.threshold as number)
          : (options[property]?.threshold as () => number)();

      if (
        options[property]?.compare === "greater"
          ? value > threshold
          : value < threshold
      ) {
